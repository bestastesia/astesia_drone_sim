...
\usepackage{fontspec}
% AR PL UMing: full CJK + Latin coverage, Light weight
% FakeBold for section headings (AR PL fonts have no real Bold)
\setmainfont{AR PL UMing CN}[
  BoldFont={AR PL UMing CN},
  BoldFeatures={FakeBold=2.5},
]
\setsansfont{AR PL UKai CN}[
  BoldFont={AR PL UKai CN},
  BoldFeatures={FakeBold=2.5},
]
...
\href{https://github.com/bestastesia/astesia_drone_sim}
     {\texttt{github.com/bestastesia/astesia\_drone\_sim}}\\[4pt]
演示视频共 10 段，覆盖全部验收场景及 bonus 测试。
\end{minipage}
\end{center}
\vspace{10pt}

\begin{tcolorbox}[colback=yellow!5, colframe=yellow!60, title={演示视频路径}]
  \small\texttt{/media/astesia/OS/Users/16021/Desktop/保研\_套磁资料/}\\
  \texttt{0中山大学/中山考核面试/}
\end{tcolorbox}
...
\toprule
\textbf{\#} & \textbf{场景} & \textbf{结果} & \textbf{演示视频} \\
\midrule
...
所有视频位于：\texttt{/media/astesia/OS/Users/16021/Desktop/} \\
\texttt{保研\_套磁资料/0中山大学/中山考核面试/}
...
Claude Code (Anthropic，claude-opus 模型)——用于架构设计、代码生成、调试、单元测试与文档。人类负责：核心公式物理核对、参数最终调整、障碍物布局设计、验收测试执行、报告撰写。
...
\section*{致谢}
% ==============================================================================
感谢以下开源项目的启发：pengyu\_sim (ROS1 动力学)、MARSIM (点云感知)、Eigen3 (线性代数)、Chart.js (地面站可视化)。
